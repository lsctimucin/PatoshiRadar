...
from round3_watch import check_transfers, clear_mint
...
TOKEN_ACCOUNT_REFRESH_EVERY = int(os.getenv("V52_LONG_TOKEN_ACCOUNT_REFRESH_EVERY", "4"))
...
def _rpc(method, params):
    ...
    try:
        response = requests.post(RPC_URL, json=payload, timeout=HTTP_TIMEOUT)
...
def add_token(mint, name="", symbol="", creator="", initial_seen_signatures=None):
    if not mint:
        return False
    initial_seen = {s for s in (initial_seen_signatures or []) if s}
    with _lock:
        if mint in _watch_tokens:
            return False
        if len(_watch_tokens) >= MAX_LONG_WATCHES:
            log(f"⚠️ V5.2 LONG WATCH LIMIT => {mint} | max={MAX_LONG_WATCHES}")
            return False
        _watch_tokens[mint] = {
            "mint": mint,
    ...
    log(f"🛰️ V5.2 LONG TRANSFER WATCH BAŞLADI => {mint} | {LONG_WATCH_SECONDS}s")
    return True

def _callback_payload(item, event, round3=False):
    return {
        "round3": round3,
        "mint": item["mint"],
        "name": item["name"],
...
def _emit_alert(item, event):
    ...
    if _alert_callback is None:
        return
    try:
        _alert_callback(_callback_payload(item, event, round3=False))
    except Exception as exc:
        log(f"❌ V5.2 LONG TRANSFER CALLBACK => {exc}")

def _emit_round3_alert(item, event):
    if _alert_callback is None or not event:
        return
    try:
        _alert_callback(_callback_payload(item, event, round3=True))
    except Exception as exc:
        log(f"❌ V5.2 LONG ROUND 3 CALLBACK => {exc}")
...
def _process_token(mint):
    ...
    try:
        ...
        # Only transfers of this already-filtered candidate mint reach this point.
        # Round-3 wallets NEVER discover/trigger a token by themselves.
        round3_event = check_transfers(item["mint"], transfers)
        if round3_event:
            _emit_round3_alert(item, round3_event)

        # Treasury remains deliberately disabled for Long Watch.
        ...
        try:
            for mint in list(_watch_tokens):
                with _lock:
                    ...
                    if not item:
                        continue
                    expired = time.time() - item["started"] >= LONG_WATCH_SECONDS
                if expired:
                    _finish_token(mint)
                    continue
                _process_token(mint)
