...
def _id():
 global req_id
 with req_lock: req_id+=1; return req_id

def _rpc(method, params):
 if not ALCHEMY_API_KEY: return None
 try:
  r=requests.post(RPC_URL,json={'jsonrpc':'2.0','id':_id(),'method':method,'params':params},timeout=10)
  if r.status_code==429: log(f'⚠️ ALCHEMY RATE LIMIT => {method}'); return None
  r.raise_for_status(); d=r.json();
  if 'error' in d: log(f"❌ ALCHEMY RPC => {d['error']}"); return None
  return d.get('result')
 except Exception as e: log(f'⚠️ ALCHEMY HTTP => {method} | {e}'); return None
...
def _event(tx):
 text=' '.join(_strings(tx)); programs=set()
...
def _process(mint):
 with lock: item=watch_tokens.get(mint)
 if not item: return
 # Activity watch
 ...
 for row in reversed(rows):
  ...
  item['seen'].add(sig); tx=_rpc('getTransaction',[sig,{'encoding':'jsonParsed','commitment':'confirmed','maxSupportedTransactionVersion':0}])
  if not tx: continue
 ...
 try:
  ...
  if transfers:
   ...
   for x in classified:
    if x['signature'] not in existing: item['transfer_summary'].append(x); existing.add(x['signature'])
 except Exception as e: log(f'⚠️ V5.2 TRANSFER WATCH => {e}')

def _finish(mint):
 with lock: item=watch_tokens.pop(mint,None)
 if not item: return
 result={'mint':item['mint'],'name':item['name'],'symbol':item['symbol'],'creator':item['creator'],'lp_detected':item['lp'],'dex_detected':bool(item['dex']),'dex_name':item['dex'],'buy_detected':item['buy'],'first_buy_signature':item['buy_sig'],'lp_signature':item['lp_sig'],'dex_signature':item['dex_sig'],'transfer_summary':item['transfer_summary'],'elapsed_seconds':int(time.time()-item['started'])}
 if callback:
  try: callback(result)
...
def start():
 ...
 if running: return
 if not ALCHEMY_API_KEY: log('❌ ALCHEMY_API_KEY yok; watch başlatılmadı.'); return
 running=True; worker_thread=threading.Thread(target=worker,daemon=True,name='PatoshiV52Watch'); worker_thread.start(); log('📡 V5.2 ACTIVITY + TRANSFER WATCH AKTİF')

def stop():
 global running; running=False

def status():
 with lock: return {k:dict(v) for k,v in watch_tokens.items()}
